"""OrganizationsExtract node for extracting organizations from text using LLM."""
...
class OrganizationsExtractInput(BaseModel):
    """Input schema - expects to find text in namespaced state."""
    model_config = ConfigDict(extra="allow")


class OrganizationsExtractParams(BaseModel):
    """Parameters for OrganizationsExtract node."""
    model: str = Field(
...
class OrgMention(BaseModel):
    """A mention of an organization in the text (no quote for organizations)."""
    text: str = Field(description="The text containing the mention")


class Organization(BaseModel):
    """An organization extracted from text."""
    name: str = Field(description="Organization name")
    type: str = Field(default="other", description="Organization type (snake_case)")
    role_in_story: str = Field(description="Brief description of the organization's role")
    mentions: List[OrgMention] = Field(description="List of mentions of this organization")
    model_config = ConfigDict(extra="allow")


class OrganizationsExtractOutput(BaseModel):
    """Output schema - returns extracted organizations and preserves input state."""
    model_config = ConfigDict(extra="allow")
    text: str = Field(description="Original input text")
    organizations: List[Organization] = Field(description="List of extracted organizations")


@register("OrganizationsExtract")
class OrganizationsExtract(
    NodeBase[OrganizationsExtractInput, OrganizationsExtractOutput, OrganizationsExtractParams]
):
    """Node for extracting organizations from text using LLM."""

    name = "OrganizationsExtract"
    version = "0.1.0"
    category = "extraction"

    Input = OrganizationsExtractInput
    Output = OrganizationsExtractOutput
    Params = OrganizationsExtractParams
    ...
    async def run(
        self,
        inp: OrganizationsExtractInput,
        params: OrganizationsExtractParams,
        ctx: RunContext,
    ) -> OrganizationsExtractOutput:
        ...
        try:
            response_text = await asyncio.wait_for(
                asyncio.to_thread(
                    ...
                    model=params.model,
                    system_message=(
                        "You are a specialized AI assistant for extracting organizations from text. "
                        "For sports_team: use full franchise names (e.g. Chicago Cubs) or full college squad names "
                        "(e.g. Butler men's basketball team) when the article's dominant sport and context support it; "
                        "apply that sport to all team mentions unless the text explicitly names another sport. "
                        "Do not invent city or sport when the text does not support it. Return only valid JSON."
                    ),
                    force_json=True,
                    temperature=0.0,
            ...
            raise TimeoutError(
                f"OrganizationsExtract LLM call exceeded timeout of {effective_timeout}s"
            )
            ...
            for org_data in orgs_data:
                ...
                for field in required_fields:
                    if field not in org_data:
                        raise ValueError(f"Missing required field '{field}' in organization data")
                if not isinstance(org_data.get("mentions"), list):
                    raise ValueError("'mentions' field must be a list")

                mentions_list = []
                for m in org_data["mentions"]:
                    if not isinstance(m, dict):
                        continue
                    text_val = m.get("text", "").strip()
                    mentions_list.append(OrgMention(text=text_val))

                org_type = org_data.get("type", "other")
                if isinstance(org_type, str):
                    org_type = org_type.strip() or "other"
                else:
                    org_type = "other"

                organizations.append(
                    Organization(
                        name=str(org_data["name"]).strip(),
                        type=org_type,
                        role_in_story=str(org_data.get("role_in_story", "")),
                        mentions=mentions_list,
                    )
                )

        except (json.JSONDecodeError, ValueError, TypeError) as e:
            raise ValueError(f"Failed to parse LLM response as organizations data: {e}")
        ...
        if isinstance(response_data, dict):
            for key, value in response_data.items():
                if key != "organizations" and key not in output_data:
                    output_data[key] = value
        ...
        return OrganizationsExtractOutput(**output_data)
