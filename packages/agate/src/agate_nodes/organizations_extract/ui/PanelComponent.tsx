...
interface OrganizationsExtractPanelProps {
  node: any
...
export default function OrganizationsExtractPanel({
  node,
  onChange,
  onRun,
  running,
  ...
  editMode,
  setNodes
}: OrganizationsExtractPanelProps) {
      ...
      <div className="space-y-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            This node uses an LLM to extract organizations (institutions, agencies, companies, schools) from text. Use placeholders like {`{text}`} in your prompt.
          </p>
        </div>
      </div>
      ...
      <div className="pt-4 border-t">
        ...
        <div className="pt-2">
          ...
          {editMode && setNodes ? (
            ...
            <div className="mt-2 p-2 bg-muted rounded max-h-48 overflow-y-auto">
              <pre className="text-xs whitespace-pre-wrap font-mono">
                {node.data?.json_format || nodeMetadata.defaultParams?.json_format || '{ "organizations": [] }'}
              </pre>
            </div>
          )}
        </div>
        ...
        <div className="pt-4 border-t">
          ...
          <div className="mt-2 space-y-2">
            <div className="text-xs text-muted-foreground">
              Organizations found: {latestData.organizations.length}
            </div>
            {latestData.organizations.length > 0 && (
              <div>
                <Label className="text-xs font-medium">Sample Organizations:</Label>
                <div className="mt-1 space-y-1 max-h-32 overflow-y-auto">
                  {latestData.organizations.slice(0, 3).map((org: any, index: number) => (
                    <div key={index} className="text-xs p-2 bg-muted rounded">
                      ...
                      {org.role_in_story && (
                        <div className="text-muted-foreground mt-1">{org.role_in_story}</div>
                      )}
                      {org.mentions && org.mentions.length > 0 && (
                        <div className="text-muted-foreground mt-1 text-xs">
                          {org.mentions.length} mention{org.mentions.length !== 1 ? 's' : ''}
                        </div>
                      )}
                    </div>
                  ))}
